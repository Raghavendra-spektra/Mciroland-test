...
SCRIPT="/home/labuser/create-users.sh"
REPORT="/home/labuser/reports/UserCreationReport.txt"

[ -f "$SCRIPT" ] || PASS=false

for USER in employee1 employee2 employee3
do
...
for USER in employee1 employee2 employee3
do
    id "$USER" >/dev/null 2>&1 || PASS=false
    [ -d "/home/$USER" ] || PASS=false
done

[ -f "$REPORT" ] || PASS=false
[ -s "$REPORT" ] || PASS=false

grep -iq "employee1" "$REPORT" || PASS=false
grep -iq "employee2" "$REPORT" || PASS=false
grep -iq "employee3" "$REPORT" || PASS=false

grep -Eiq "created|already exists|failed|status" "$REPORT" || PASS=false

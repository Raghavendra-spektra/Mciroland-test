...
do {
    try {
        ...
        $commands = @(
            'PASS=true',
            'GROUP_GID=$(getent group devops | cut -d: -f3)',
            'if [ "$GROUP_GID" != "5001" ]; then PASS=false; fi',
            'MAX_AGE=$(chage -l devadmin | grep "Maximum number of days" | awk -F: ''{print $2}'' | xargs)',
            'if [ "$MAX_AGE" != "30" ]; then PASS=false; fi',
            'passwd -S devviewer | grep -q " L " || PASS=false',

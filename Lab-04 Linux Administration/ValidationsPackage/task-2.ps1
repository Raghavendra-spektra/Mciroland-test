...
do {
    try {
        ...
        $commands = @(
            'PASS=true',
            'id jsmith >/dev/null 2>&1 || PASS=false',
            'id -nG jsmith | grep -qw devops || PASS=false',
            'USER_SHELL=$(getent passwd jsmith | cut -d: -f7)',
            'if [ "$USER_SHELL" != "/bin/sh" ]; then PASS=false; fi',
            '[ -f /tmp/jsmith_groups.txt ] || PASS=false',
            'grep -q jsmith /tmp/jsmith_groups.txt || PASS=false',
            'if $PASS; then echo "TASK-2 PASSED"; exit 0; else echo "TASK-2 FAILED"; exit 1; fi'
        )

...
do {
    try {
        ...
        $commands = @(
            'PASS=true',
        'dpkg -s nginx >/dev/null 2>&1 || PASS=false',
        '[ -d /etc/nginx ] || PASS=false',
        'apt-mark showhold | grep -qw nginx || PASS=false',
        'if $PASS; then echo "TASK-4 PASSED"; exit 0; else echo "TASK-4 FAILED"; exit 1; fi'
        )

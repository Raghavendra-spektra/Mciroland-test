...
do {
    try {
        ...
        $commands = @(
            'PASS=true',
            '[ -f /tmp/network_report.txt ] || PASS=false',
            'grep -q "default" /tmp/network_report.txt || PASS=false',
            'grep -Ei "google.com|server|address" /tmp/network_report.txt >/dev/null 2>&1 || PASS=false',
            'if $PASS; then echo "TASK-8 PASSED"; exit 0; else echo "TASK-8 FAILED"; exit 1; fi'
        )
        ...
        if ($status -eq "Success") {
            ...
            $message = @{
                Status  = "Succeeded"
                Message = "TASK-8 validation passed."
            } | ConvertTo-Json
            ...
            $message = @{
                Status  = "Failed"
                Message = "TASK-8 validation failed."
            } | ConvertTo-Json
